...
const Community = () => {
  ...
  return !loading ? (
          ...
          <div key={index} className='relative group inline-block pl-3 pt-3 w-full sm:w-1/2 lg:w-1/3'>
            ...
            <div className='absolute bottom-0 left-3 right-0 top-0 flex gap-2 items-end justify-end group-hover:justify-between p-3 group-hover:bg-gradient-to-b from-transparent to-black/80 text-white rounded-lg'>
              ...
              <div className='flex gap-1 items-center'>
                ...
                <Heart onClick={()=> imageLikeToggle(creation.id)} className={`w-5 h-5 hover:scale-110 cursor-pointer ${creation.likes.includes(user.id) ? 'fill-red-500 text-red-600' : 'text-white'}`}/>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className='flex justify-center items-center h-full'>
      <span className='w-10 h-10 my-1 rounded-ful border-3 
