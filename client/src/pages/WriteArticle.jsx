...
const WriteArticle = () => {
  ...
  const onSubmitHandler =async (e)=>{
    ...
    try {
      setLoading(true)
      const prompt =`Write an article on the topic "${input}" with a length of ${selectedLength.text} words.`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4  text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200 '>
          ...
          <p className='mt-4   text-sm font-medium'>Article Keywords</p>
...

          {
            articleLength.map((item, index)=>(
              ...
              ${selectedLength.text ===item.text ? 'bg-blue-50 text-blue-700': 'text-gray-500 border-gray300' }`} key={index}>
                {item.text} 
              </span>
          ))}
          ...
          from-[#226BFF] to-[#65ADFF] text-white  px-4 py-2 mt-6 text-sm rounded-lg  cursor-pointer '>
            ...
            border-t-transparent animate-spin'> </span>:    <Edit className='w-5'/>}
